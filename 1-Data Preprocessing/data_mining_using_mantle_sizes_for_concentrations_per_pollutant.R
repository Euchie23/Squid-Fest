...
#   y <- paste0(x[1],"<img src=", "OCicons//", x[1], ".png", " width='17'/>")
#   return(y)
# }
...
#   if(file.exists(paste0("HMicons//",x[1],".png"))&file.exists(paste0("HMicons//",x[1],"1",".png"))&file.exists(paste0("HMicons//",x[1],"2",".png"))){
#     y <- paste0(" ","<img src=", "HMicons//", x[1], ".png", " width='17'/>"," ", " ","<img src=","HMicons//", x[1],"1",".png"," width='17'/>"," ", " ","<img src=","HMicons//", x[1],"2",".png"," width='17'/>")
#   }else if(file.exists(paste0("HMicons//",x[1],".png"))&file.exists(paste0("HMicons//",x[1],"1",".png"))){
...
data_mining_using_mantle_length_for_concentrations_per_pollutant <- function (data_list, quantile_level_for_outliers, axis_modification_multiplier, variable, remove.zeroes = FALSE){
  ...
  for (h in 1:length(number_range)) {
   pollutant <- colnames(dataset_with_numerical_values[,c(number_range)])[h]
    ...
    if (!(variable %in% c("Month_of_Capture", "dta_km", "dtfl_km"))){
      stop("Please use one of these variables, copy and past if need be:\n Month_of_Capture\ndta_km\n dtfl_km")# This piece of code accounts for if any other variable (x variable) is chosen other than these three or in the even that they were typed wrong. It send out a messsage to the user.
    }
    ...
    if (nrow(long_dataset) != 0) {
      ...
      print(long_dataset[,c(17:21)])
      print(pvalues_accumulated)
      print(nrow(long_dataset))
      ...
      #     {if (nrow(pvalues_accumulated) != 0) geom_text(data=pvalues_accumulated, mapping = aes(label = paste(rho, pvalues, sep = ",")),inherit.aes = FALSE, hjust = -0.7, size = 3.5, fontface = "italic", position = position_dodge(width = .1), check_overlap = FALSE)}
      #   
  ...
  # if (length(list1) > 0) names(list1) <- list1names  
  # if (length(list2) > 0) names(list2) <- list2names 
...
elements_from_data_mining_using_mantle_length_per_pollutant <- data_mining_using_mantle_length_for_concentrations_per_pollutant (datasets_for_heavy_metals_data_distribution, quantile_level_for_outliers = 0.95, axis_modification_multiplier = 0.65, variable='Month_of_Capture', remove.zeroes = TRUE)
