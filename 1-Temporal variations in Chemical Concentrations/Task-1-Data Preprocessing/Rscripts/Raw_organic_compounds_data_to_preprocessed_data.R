...
files <- list.files(path = "Datasets/Organic Compounds Raw Data/2021_OC/")
path <- "Datasets/Organic Compounds Raw Data/2021_OC/"

dry_weight <- read.csv("Datasets/Organic Compounds Raw Data/Dry weight data /2021dryweight_SM.csv", header= TRUE)

#distance to Argentina and Falkland Islands.distance to land (dtl)
distance_to_land <- read.csv("Datasets/Squid Catch Data/Distance_to_land.csv", header= TRUE)

#SQUID CATCH DATA...CHANGE BASED ON THE YEAR YOU ARE PROCESSING, Basic information = bi
squid_info <- read.csv("Datasets/Squid Catch Data/2021_catch_data.csv", header= TRUE)
