...
#2 FUNCTIONS TO HELP MODIFY SAMPLE IDS FOR DRYWEIGHT DATASET
sample_area_dry_weight <- function(x){#padding 0 to first two digits
  x[1] =  gsub("-", "_",  x[1])
  print(paste(x[1]))
  if (str_detect(substring(x[1],1,2), '_') == TRUE){
    sk=print(paste('0',substring(x[1],1,)))
...
#FUNCTIONS TO ADD DRYWEIGHT TO CONCENTRATION DATASET
...
ocnames <- c('Adipic_acid','Caprolactam','Chlorpyrifos','Ibuprofen')
...
#   return(x)
# }
OC <- lapply(OCcsv, data_manipulation1)
...
#   return(x)
# }
OC2<-lapply(OC1, Data_manipulation2)

#check point 2 (Sm2 list (any dataset)) 
#------------------------------------------------------------------------------------------
  
  
#ADDING SAMPLE SEPARATION DATA (dry weight) TO CONCENTRATION DATASET AND CALCULATIING CONCETRATION MG/KG----
...
OC.2 <- data.frame(matrix(NA, nrow=nrow(OC2[[1]]), ncol=length(OC2)+5))
OC.2[,1]<-OC3[[1]][,1]
OC.2[,2]<-OC3[[1]][,2]
OC.2[,3]<-OC3[[1]][,9]
OC.2[,4]<-OC3[[1]][,8]
OC.2[,5]<-OC3[[1]][,7]
colnames(OC.2)[1]<-colnames(OC2[[1]])[1]
colnames(OC.2)[2]<-colnames(OC2[[1]])[2]
colnames(OC.2)[3]<-colnames(OC2[[1]])[9]
colnames(OC.2)[4]<-colnames(OC2[[1]])[8]
colnames(OC.2)[5]<-colnames(OC2[[1]])[7]
colnames(OC.2)[1:length(names(OC3))+5]<-names(OC3)
...
colnames(x)[c(1,3)] <- c("ID", "DW")
   ...
   Final_res_OC = "Results/Final_OCresults_mgkg.csv"
...
path <- "Organic Compounds Raw Data/2021_OC/"

dry_weight <- read.csv("Organic Compounds Raw Data/Dry weight data /2021dryweight_SM.csv", header= TRUE)
...
squid_info <- read.csv("Squid Catch Data/2021_catch_data.csv", header= TRUE)
