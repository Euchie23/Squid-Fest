...
hm1.1 = sort(colnames(hm1[,c(17:26)]))
hm1.2 <-hm1 %>% relocate(1:16, hm1.1)
...
sm2.1<- read.csv("Chem_lab_excel_files /R_Exported_excel:CSV_files/final results HM & SM/SM_results_forTSUNGHAN.csv", header= TRUE)
colnames(sm2.1)[4] <- 'Area'
#sm2.2 <-as.data.frame(sm2.1 %>% relocate(Area, .before = ID_num))
sm2.1[,4] <- paste0(sm2.1[,4], ".")
sm <- as.data.frame(t(apply(sm2.1, MARGIN = 1, dkc)))
sm$Area <- gsub("\\.", "",sm$Area)
...
# hmfull <- as.data.frame(t(apply(hm1.2, MARGIN = 1, dcc)))
# hmfull$Area <- gsub("\\.", "",hmfull$Area)
# hmfull[,c(17:26)] <- lapply(hmfull[,c(17:26)], gsub, pattern = ".*BLOQ.*", replacement = 'BLOQ')
# hmfull[,c(17:26)] <- lapply(hmfull[,c(17:26)], gsub, pattern = ".*BB.*", replacement = 'BB')
# hmfull[,c(17:26)] <- lapply(hmfull[,c(17:26)], gsub, pattern = "^0$", replacement = 'BLOD')
# hmfull[,c(17:26)]  <- lapply(hmfull[,c(17:26)] , gsub, pattern = "N/A", replacement = 'BLOD')
# hmfull[,c(17:26)] <- lapply(hmfull[,c(17:26)], gsub, pattern = "[0-9.]+", replacement = "Detected")
# hmdis <- hmfull
...
hmfull <- as.data.frame(t(apply(hm1.2, MARGIN = 1, dcc)))
hmfull$Area <- gsub("\\.", "",hmfull$Area)
hmfull[,c(17:26)] <- lapply(hmfull[,c(17:26)], gsub, pattern = ".*BLOQ.*", replacement = paste('BB+BLOQ'))
hmfull[,c(17:26)] <- lapply(hmfull[,c(17:26)], gsub, pattern = ".*BB.*", replacement = paste('BB+BLOQ'))
hmfull[,c(17:26)] <- lapply(hmfull[,c(17:26)], gsub, pattern = "^0$", replacement = 'BLOD')
hmfull[,c(17:26)]  <- lapply(hmfull[,c(17:26)] , gsub, pattern = "N/A", replacement = 'BLOD')
hmfull[,c(17:26)] <- lapply(hmfull[,c(17:26)], gsub, pattern = "[0-9.]+", replacement = "Detected")
hmdis <- hmfull
...
sm2[,'Aminobenzoic_acid'] <- as.character(sm2[,'Aminobenzoic_acid'])
sm2[,4] <- paste0(sm2[,4], ".")
colnames(sm2)[4]<- "Area"
sm1 <- as.data.frame(t(apply(sm2, MARGIN = 1, dkc)))
sm1$Area <- gsub("\\.", "",sm1$Area)
sm <- sm1 %>% relocate(Area, .before = ID_num)
sm[,c(17:30)] <- lapply(sm[,c(17:30)], gsub, pattern = ".*BLOQ.*", replacement = 'BLOQ')
sm[,c(17:30)] <- lapply(sm[,c(17:30)], gsub, pattern = "N/A", replacement ='BLOD')
sm[,c(17:30)] <- lapply(sm[,c(17:30)], gsub, pattern = "^0$", replacement ='BLOD')
sm[,c(17:30)] <- lapply(sm[,c(17:30)], gsub, pattern = "[0-9.]+", replacement ='Detected')
sm[,c(17:30)] <- lapply(sm[,c(17:30)], gsub, pattern = "DetectedE-Detected", replacement ='Detected')
sm[,'Tissue'] <- tolower(sm[,'Tissue'])
smdis <- sm 
