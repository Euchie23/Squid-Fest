    ...
    if(nrow(subset_by_year)!=0){
      for (i in 1:length(sizes)){
        ...
        if(nrow(size_dataframe)<2|all(size_dataframe[-1,'concentrations'] == size_dataframe[1,'concentrations']|max(size_dataframe$concentrations)== 0)==TRUE){ 
          ...
          coefficient_results_1[1,7] <- ifelse(max(size_dataframe$concentrations)|length(size_dataframe$concentrations) == 0, 0, max(size_dataframe$concentrations)) # Max concentration for y-axis
        ...
        # If the maximum concentration in the size dataframe is not 0 then go to perform the correlation test for large sized squids.
          ...
          coefficient_results_1[1, 2] <- years[s]
          coefficient_results_1[1, 3] <- signif((y_upper_limit * 0.95), 3)  #X-coordinate for rho coefficient and p-value
          
          
          # Adjust the variable depending on the year and the variable type
          coefficient_results_1[1, 4] <- adjust_variable(subset_by_year, sizes[i], years[s])
          
          # Store the rho and p-values from the correlation test
          coefficient_results_1[1, 5] <- coefficient0$estimate
          coefficient_results_1[1, 6] <- coefficient0$p.value
          coefficient_results_1[1, 7] <- y_upper_limit  # Store the calculated yscale
          coefficient_results_1[1, 8] <- unique(size_dataframe$size)
...
#Calling Main Function. All arguments except remove.zeroes (default is set at False) are required and user has to choose between datasets_for_organic_compounds or datasets_for_heavy_metals. Also choose a multiplier to help adjust the y_axis since some plots may not show results clearly due to the compression effect or scale distortion of the outliers. For example: The 0.80 means that the upper limit of the y axis will be set at 0.80 * the minimum outlier from the upper quantile for each variable (distance to Argentina_km, distance to the Falkland Islands_km and month of capture). This function shows how pollution levels collectively change in different tissues of the squids depending on the aforementioned three variables. Users can also remove all zeroes and focus on only the detected concentrations or keep them. The results are saved in size_comparison_results list.
collective_size_comparison_results <- comparing_sizes_collectively (datasets_for_heavy_metals, axis_modification_multiplier = 0.80, remove.zeroes = TRUE)
