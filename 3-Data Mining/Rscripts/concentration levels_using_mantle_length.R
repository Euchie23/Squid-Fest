...
markdownfuncsm <- function(x){
  ...
  y <- paste0("<img src=", "SMicons//", x[1], ".png", " width='17'/>")
  return(y)
}
#For Heavy Metals:
...
forcof <- function (x, per_of_outl_for_yaxis=.75){
  #x=dfnz
    ...
    if(nrow(year)!=0){
      ...
      #   filtbyyear <- filter(filtbytis, Year == yr[s])
        ...
        if(nrow(sizedf)<2|all(sizedf[-1,'conc'] == sizedf[1,'conc']|max(sizedf$conc)== 0)==TRUE){
          ...
          ctr0[1,4] <- signif((yscale*.95), 3)# X-coord for r coefficient and p-value
          if(unique(year$vars) == 'dtfl_km' & unique(year$Year)== '2019'){
            ctr0[1,5] <- max(year$distance)*.75
          }else if (unique(year$vars) == 'dtfl_km' & unique(year$Year)== '2020'){
            ctr0[1,5] <- max(year$distance)*.85
          }else if (unique(year$vars) == 'dtfl_km' & unique(year$Year)== '2021'){
            ctr0[1,5] <- max(year$distance)*.5
          }else if (unique(year$vars) == 'dta_km' & unique(year$Year)== '2019'){
            ctr0[1,5] <- min(year$distance)
          }else if (unique(year$vars) == 'dta_km' & unique(year$Year)== '2020'){
            ctr0[1,5] <- min(year$distance)
         }else{
            ctr0[1,5] <- max(year$distance)*.5
          }
...
cluml <- function(y, remove.zeroes=FALSE){
  ...
  if(str_detect(colnames(y[17]),'Ag')==TRUE){
    ...
    numrang <- 17:26
    ndf0 <-y[,c(numrang,3, 6, 15, 16, 27)]# streamlining dataframe for efficiency change back to 26 from 30 for heavy metals
  }else{
    num <-round(1/3* nrow(y))
    y <- y[order(y$Mantle_Length_mm),]
    y$size=NA
    ...
    numrang <- 17:30
    ndf0 <-y[,c(numrang,3, 6, 15, 16, 31)]# streamlining dataframe for efficiency change back to 26 from 30 for heavy metals
  }
  h<-1
  dtafl <-y[,c(15, 16)]
  while(h != 3){
  distance <- colnames(dtafl[h])
  print(distance)
  #print(h)
  for(m in 1:length(numrang)){
    ...
    if(remove.zeroes==FALSE){
      dfnz<- ndf %>% pivot_longer(paste(hm), names_to = "SM", values_to = "conc")%>%pivot_longer (!! rlang::sym(paste0(distance)), names_to = "vars", values_to = "distance")
    }else{
      dfnz<- ndf %>% pivot_longer(paste(hm), names_to = "SM", values_to = "conc")%>%pivot_longer (!! rlang::sym(paste0(distance)), names_to = "vars", values_to = "distance")%>%subset(conc !=0)
       # print(dfnz[,c(9:21)])
      # print(nrow(dfnz))
    }
    if(nrow(dfnz)!= 0){
  ...
  #For graphs----
    ...
    if(distance =='dta_km'){
        plt <-dfnz %>% ggplot(aes(distance, conc, colour = size, group=size)) +
          ...
          labs(title = paste(icons_markdown[m], '<B>',hm,'::','</B>', distance,"Vs Conc mg/kg using size",sep =" "),
               y = "Concentration mg/kg", x = paste(distance))+
        ...
        name0 <- paste(hm,"plots", distance, sep = "")
      ...
      plt <-dfnz %>% ggplot(aes(distance, conc, colour = size, group=size)) +
        ...
        name1 <- paste(hm,"plots", distance, sep = "")
  ...
  return(list (dta_km=list0, dtfl_km=list1))
}
clumlm <- cluml (sm1, remove.zeroes = TRUE)

for(i in 1:length(clumlm$dtfl_km)){
  # if(names(clumlm$dta_km)[i]== "Estradiolplotsdta_km"|names(clumlm$dta_km)[i]== "Ethylene_glycolplotsdta_km"){
  #   next
  # }else{
  png(paste0(names(clumlm$dtfl_km)[i], ".png"), width = 1400, height = 800)
  plot(clumlm$dtfl_km[[i]],title = names(clumlm$dtfl_km)[i])
  dev.off()
#}
}