...
process_dataset <- function(data, keep_LOQ_values=FALSE) {
  ...
  if (grepl("Fe|Ag", colnames(data)[16])) {
    ...
    # FUNCTIONS TO HELP MODIFY ID NUMBERS FOR FURTHER ANALYSIS (pollutant)
...
#Helper function for prepping markdown for ggplot. This is used to turn the url text into an image on the graph.
...
get_y_axis_upper_limit <- function (dataset, axis_modification_multiplier){
  ...
  if(length(outliers)!=0){
    ...
    y_upper_limit <-minimum_outlier*axis_modification_multiplier # Set the upper limit for the y-axis based on the smallest outlier
...
temporal_comparisons_between_genders <- function (data_list, axis_modification_multiplier, variable, remove.zeroes = FALSE){
...
# streamlining dataframe for efficiency change back to 26 from 30 for heavy metals
  ...
  for(i in 1:length(tissues)) { #sub-setting per tissue for each pollutant
    ...
    if(nrow(subsetted_datatset_long_by_tissue)==0){
      ...
      #Function used to get y_axis_upper_limit from subsetted_datatset_long_by_tissue
       y_axis_upper_limit <- get_y_axis_upper_limit(subsetted_datatset_long_by_tissue, axis_modification_multiplier)
...
# Below condition used to run normality test for dataset after sub-setting per year for each pollutant after sub-setting by tissue and gender.      
  ...
  print(pollutant)
  print(accumulated_results_GBY)
  print(accumulated_results_GWY)
...
gender_temporal_comparisons_results <- temporal_comparisons_between_genders(datasets_for_organic_compounds, 
                                                                            axis_modification_multiplier=0.95,
                                                                            remove.zeroes = TRUE)
...
save_graphs <- function(graph_list) {
  ...
  if (grepl("Fe|Ag", names(graph_list[["boxplots_for_pollutants"]])[[1]])) {
    output_folder <- file.path("/Users/mrnobody/Documents/GitHub/Squid-Fest/3-Data Mining/Data_mining_plots/temporal_gender_comparisons","Heavy_metals")
  }else{
    output_folder <- file.path("/Users/mrnobody/Documents/GitHub/Squid-Fest/3-Data Mining/Data_mining_plots/temporal_gender_comparisons","Organic_compounds")  
  }
